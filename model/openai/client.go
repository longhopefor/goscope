...
type Config struct {
	APIKey string
	Model  string
	// BaseURL 包含 /v1；默认为 https://api.openai.com/v1。
	BaseURL    string
...
func New(c Config) (*Client, error) {
	...
	return &Client{key: c.APIKey, name: c.Model, endpoint: strings.TrimRight(c.BaseURL, "/") + "/chat/completions", http: &h}, nil
}
...
func (c *Client) send(ctx context.Context, r model.Request, stream bool) (*http.Response, error) {
	...
	if res.StatusCode < 200 || res.StatusCode >= 300 {
		res.Body.Close()
		return nil, &HTTPError{StatusCode: res.StatusCode, RequestID: res.Header.Get("x-request-id")}
	}
	return res, nil
}
